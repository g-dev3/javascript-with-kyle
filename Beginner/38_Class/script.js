...
function playNote() {
  NOTE_DETAILS.forEach((n) => {
    ...
    if (n.ascillator != null) {
      n.ascillator.stop();
      n.ascillator.disconnect();
    }
  });

  const activeNote = NOTE_DETAILS.filter((n) => n.active);
  const gain = 1 / activeNote.length;
  activeNote.forEach((n) => {
    startNote(n, gain);
  });
}
...
function startNote(noteDetail, gain) {
  ...
  const ascillator = audioContext.createOscillator();
  ascillator.frequency.value = noteDetail.frequency;
  ascillator.type = "sine";
  ascillator.connect(gainNode).connect(audioContext.destination);
  ascillator.start();
  noteDetail.ascillator = ascillator;
}
